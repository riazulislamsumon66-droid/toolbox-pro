...
import { checkTitleAndMeta } from "@/tools/seo"
...
export default function TitleMetaCheckerPage() {
  const [html, setHtml] = useState("")
  const [results, setResults] = useState<ReturnType<typeof checkTitleAndMeta> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkTitleAndMeta(data.html))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkTitleAndMeta(html))
  }

  return (
    <ToolPage title="Title & Meta Checker" description="Check page title, meta description, canonical URL, viewport, and charset for SEO." icon="📝" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="📝 Analyze" />
        ) : (
          <>
