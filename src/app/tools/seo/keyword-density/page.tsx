...
export default function KeywordDensityPage() {
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkKeywordDensity(data.html, targetKeyword || undefined))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkKeywordDensity(html, targetKeyword || undefined))
  }

  return (
    <ToolPage title="Keyword Density" description="Analyze keyword frequency and density in content." icon="📊" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <>
            <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="📊 Analyze Keywords" />
            <div><label className="block text-sm font-medium text-gray-400 mb-2">Target keyword (optional):</label><input type="text" value={targetKeyword} onChange={(e) => setTargetKeyword(e.target.value)} placeholder="e.g., SEO tools" className="w-full p-3 bg-gray-900 border border-gray-800 rounded-xl text-gray-100 placeholder-gray-500 focus:outline-none focus:border-purple-500 transition-all" /></div>
          </>
        ) : (
          <>
