...
import { checkLinks } from "@/tools/seo"
...
export default function LinkAnalyzerPage() {
  ...
  const [results, setResults] = useState<ReturnType<typeof checkLinks> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkLinks(data.html, data.url))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkLinks(html))
  }

  return (
    <ToolPage title="Link Analyzer" description="Analyze internal, external, nofollow links and anchor text." icon="🔗" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="🔗 Analyze Links" />
        ) : (
          <>
