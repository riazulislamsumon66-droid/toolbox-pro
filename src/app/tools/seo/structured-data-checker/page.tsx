...
import { checkStructuredData } from "@/tools/seo"
...
export default function StructuredDataPage() {
  ...
  const [results, setResults] = useState<ReturnType<typeof checkStructuredData> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkStructuredData(data.html))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkStructuredData(html))
  }

  return (
    <ToolPage title="Structured Data" description="Check JSON-LD and microdata schema markup." icon="{ }" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="{ } Check Schema" />
        ) : (
          <>
