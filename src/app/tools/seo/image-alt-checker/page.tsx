...
import { checkImageAltText } from "@/tools/seo"
...
export default function ImageAltCheckerPage() {
  ...
  const [results, setResults] = useState<ReturnType<typeof checkImageAltText> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkImageAltText(data.html))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkImageAltText(html))
  }

  return (
    <ToolPage title="Image Alt Checker" description="Find missing alt text on images — important for SEO and accessibility." icon="🖼️" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="🖼️ Check Images" />
        ) : (
          <>
