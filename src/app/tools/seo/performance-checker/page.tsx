...
import { checkPerformance } from "@/tools/seo"
...
export default function PerformanceCheckerPage() {
  ...
  const [results, setResults] = useState<ReturnType<typeof checkPerformance> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkPerformance(data.html))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkPerformance(html))
  }

  return (
    <ToolPage title="Performance Check" description="Check page size, image dimensions, scripts, and inline styles." icon="⚡" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="⚡ Check Performance" />
        ) : (
          <>
