...
import { checkMobileFriendly } from "@/tools/seo"
...
export default function MobileCheckerPage() {
  ...
  const [results, setResults] = useState<ReturnType<typeof checkMobileFriendly> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkMobileFriendly(data.html))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkMobileFriendly(html))
  }

  return (
    <ToolPage title="Mobile-Friendly Check" description="Check viewport, font sizes, fixed widths, and touch targets." icon="📱" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="📱 Check Mobile" />
        ) : (
          <>
