...
import { checkHeadings } from "@/tools/seo"
...
export default function HeadingAnalyzerPage() {
  ...
  const [results, setResults] = useState<ReturnType<typeof checkHeadings> | null>(null)
  ...
  const handleFetchAnalyze = async (url: string) => {
    ...
    if (data) {
      setHtml(data.html)
      setResults(checkHeadings(data.html))
    }
  }

  const handlePasteAnalyze = () => {
    if (!html.trim()) return
    setResults(checkHeadings(html))
  }

  return (
    <ToolPage title="Heading Structure Analyzer" description="Check H1-H6 heading hierarchy and structure for SEO." icon="📋" category="seo">
      <div className="space-y-6">
        ...
        {activeTab === "url" ? (
          <UrlInput onFetch={handleFetchAnalyze} loading={loading} buttonText="📋 Analyze Headings" />
        ) : (
          <>
